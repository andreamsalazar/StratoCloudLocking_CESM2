...
use physconst,           only: cappa, cpair, pi ! AMS: cloud locking (added pi for regional CL)
...
integer :: i_dei, i_mu, i_lambda, i_iciwp, i_iclwp, i_des, i_icswp  ! AMS: cloud locking
...
subroutine radiation_init(pbuf2d)
   ...
   ! AMS: cloud locking : allow output for cloud-locking data
   ...
   call addfld('CLDFPRIME_rad', (/ 'lev'/), 'A','fraction', 'CLDFPRIME output from radiation')
...
subroutine radiation_tend( &
   ...
   use physics_buffer, only : physics_buffer_desc, pbuf_get_field, pbuf_old_tim_idx, pbuf_set_field ! AMS: Cloud Locking
   ...
   real(r8), pointer, dimension(:,:)   :: dei, mu, lambda, iciwp, iclwp, des, icswp  ! AMS: cloud locking

   ! AMS: temporary  variables for storing prognostic cloud
    real(r8) cldtmp(pcols,pver)     ! storing temporary prognostic cloud parameters
    real(r8) cldfsnowtmp(pcols,pver)!    
    real(r8) destmp(pcols,pver)     !
    real(r8) deitmp(pcols,pver)     !
    real(r8) icswptmp(pcols,pver)   !
    real(r8) iclwptmp(pcols,pver)   !
    real(r8) iciwptmp(pcols,pver)   !
    real(r8) mutmp(pcols,pver)      !
    real(r8) lambdatmp(pcols,pver)  !

    real(r8) cld_keep(pcols,pver)     ! storing temporary prognostic cloud parameters
    real(r8) cldfsnow_keep(pcols,pver)!    
    real(r8) des_keep(pcols,pver)     !
    real(r8) dei_keep(pcols,pver)     !
    real(r8) icswp_keep(pcols,pver)   !
    real(r8) iclwp_keep(pcols,pver)   !
    real(r8) iciwp_keep(pcols,pver)   !
    real(r8) mu_keep(pcols,pver)      !
    real(r8) lambda_keep(pcols,pver)  !


    ! AMS: shape of cloud for locking

    real(r8), allocatable :: vertical_shape_CLD(:) ! vertical shape of cloud fraction to be scaled
    real(r8), allocatable :: vertical_shape_LWP(:) ! vertical shape of LWP to be scaled
    integer :: nstep 
    real(r8) :: season_sin
    real(r8) :: diurnal_sin
    real(r8) :: shift
   ...
   lchnk = state%lchnk
   ncol = state%ncol
   nstep = get_nstep() ! AMS: get current timestep number (1 every 30 seconds)

   ! AMS: set vertical shapes 

   vertical_shape_CLD=(/0.0,0.0, 0.0, 0.002, 0.47,1.0,0.738,0.313,0.076,0.005,0.0/)
   vertical_shape_LWP=(/0.0,0.0,0.0,0.088,1.0,0.768,0.31,0.054,0.0,0.0,0.0/)
...
! AMS: also read in cloud parameters that came from file for locking in a certain region


   ! AMS - save off old (unlocked values)
   des_keep      = des
   dei_keep      = dei
   mu_keep       = mu
   lambda_keep   = lambda
   ...
   icswp_keep    = icswp
   cld_keep      = cld
   cldfsnow_keep = cldfsnow

   season_sin = -0.1*sin((nstep-40*24*2)*2*pi/(365*24*2)) + 0.7


   ! AMS - locking only in subtropical strat regions (Peru + California + Nambia) and in lowest 10 levels and over ocean (land frac < 0.5)
   do i=1,ncol
   do k=1, pver
      if ((clat(i)*(180._r8/pi)).le. 0. .and. (clat(i)*(180._r8/pi)).ge. -33. .and. clon(i)*(180._r8/pi).ge. 251. .and. clon(i)*(180._r8/pi).le. 300. .and. k .ge. 22 .and. cam_in%landfrac(i) .le. 0.5_r8) then ! Peruvian coast
         shift = -(360-(clon(i)*(180._r8/pi)))/7.5 ! 7.5 degree solar movement per step
         diurnal_sin = (15./100.)*sin((nstep+shift)*2*pi/(48.))
         destmp(i,k)=0._r8
         deitmp(i,k)=0._r8
         mutmp(i,k)=6._r8
         lambdatmp(i,k)=320000._r8
         iclwptmp(i,k)=2*(season_sin/15 + 0.02*diurnal_sin)*vertical_shape_LWP(k-21)
         iciwptmp(i,k)=0._r8
         icswptmp(i,k)=0._r8
         cldtmp(i,k)=(season_sin+diurnal_sin)*vertical_shape_CLD(k-21)
         cldfsnowtmp(i,k)=(season_sin + diurnal_sin)*vertical_shape_CLD(k-21)
      else if ((clat(i)*(180._r8/pi)).le. 40. .and. (clat(i)*(180._r8/pi)).ge. 15. .and. clon(i)*(180._r8/pi).ge. 211. .and. clon(i)*(180._r8/pi).le. 250. .and. k .ge. 22 .and. cam_in%landfrac(i) .le. 0.5_r8) then ! California coast
         ! write(iulog,*) 'latitude/lon locked: ',clat(i)*(180._r8/pi), clon(i)*(180._r8/pi)
         shift = -(360- (clon(i)*(180._r8/pi)))/7.5 ! 7.5 degree solar movement per step
         diurnal_sin = (15./100.)*sin((nstep+shift)*2*pi/(48.))
         destmp(i,k)=0._r8
         deitmp(i,k)=0._r8
         mutmp(i,k)=6._r8
         lambdatmp(i,k)=320000._r8
         iclwptmp(i,k)=2.2*(season_sin/15 + 0.02*diurnal_sin)*vertical_shape_LWP(k-21)
         iciwptmp(i,k)=0._r8
         icswptmp(i,k)=0._r8
         cldtmp(i,k)=(season_sin + diurnal_sin)*vertical_shape_CLD(k-21)
         cldfsnowtmp(i,k)=1.04*(season_sin + diurnal_sin)*vertical_shape_CLD(k-21)
      else if ((clat(i)*(180._r8/pi)).le. 0. .and. (clat(i)*(180._r8/pi)).ge. -33. .and. ((clon(i)*(180._r8/pi).ge. 341. .and. clon(i)*(180._r8/pi).le. 360.) .or. (clon(i)*(180._r8/pi).ge. 0. .and. clon(i)*(180._r8/pi).le. 20.)) .and. k .ge. 22 .and. cam_in%landfrac(i) .le. 0.5_r8) then ! Nambian coast
         !write(iulog,*) 'latitude/lon locked: ',clat(i)*(180._r8/pi), clon(i)*(180._r8/pi)
         shift = -(360- (clon(i)*(180._r8/pi)))/7.5 ! 7.5 degree solar movement per step
         diurnal_sin = (15./100.)*sin((nstep+shift)*2*pi/(48.))
         destmp(i,k)=0._r8
         ...
         iciwptmp(i,k)=0._r8
         icswptmp(i,k)=0._r8
         cldtmp(i,k)=1.04*(season_sin + diurnal_sin)*vertical_shape_CLD(k-21)
         cldfsnowtmp(i,k)=(season_sin + diurnal_sin)*vertical_shape_CLD(k-21)
      else
         destmp(i,k)=des(i,k)
         deitmp(i,k)=dei(i,k)
         mutmp(i,k)=mu(i,k)
         lambdatmp(i,k)=lambda(i,k)
         iclwptmp(i,k)=iclwp(i,k)
         iciwptmp(i,k)=iciwp(i,k)
         icswptmp(i,k)=icswp(i,k) 
         cldtmp(i,k)=cld(i,k) 
         cldfsnowtmp(i,k)=cldfsnow(i,k) 
      endif
   enddo
   enddo
! EAM: do we need to specify the dimensions of locked cld & cldfsnow?
   call pbuf_set_field(pbuf,cld_idx,cldtmp, start=(/1,1,itim_old/), kount=(/pcols,pver,1/))
   call pbuf_set_field(pbuf,cldfsnow_idx,cldfsnowtmp, start=(/1,1,itim_old/), kount=(/pcols,pver,1/))
   call pbuf_set_field(pbuf,i_des,destmp)
   call pbuf_set_field(pbuf,i_dei,deitmp)
   call pbuf_set_field(pbuf,i_iciwp,iciwptmp)
   call pbuf_set_field(pbuf,i_icswp,icswptmp)
   call pbuf_set_field(pbuf,i_iclwp,iclwptmp)
   call pbuf_set_field(pbuf,i_lambda,lambdatmp)
   call pbuf_set_field(pbuf,i_mu,mutmp)

    ! AMS: output fields that will later be used as radiation boundary data
! outputting locked parameters
   ...
   call pbuf_set_field(pbuf,cld_idx,cld_keep, start=(/1,1,itim_old/), kount=(/pcols,pver,1/))
   call pbuf_set_field(pbuf,cldfsnow_idx,cldfsnow_keep, start=(/1,1,itim_old/), kount=(/pcols,pver,1/))
   call pbuf_set_field(pbuf,i_des,des_keep)
   call pbuf_set_field(pbuf,i_dei,dei_keep)
   call pbuf_set_field(pbuf,i_iciwp,iciwp_keep)
   call pbuf_set_field(pbuf,i_icswp,icswp_keep)
   call pbuf_set_field(pbuf,i_iclwp,iclwp_keep)
   call pbuf_set_field(pbuf,i_lambda,lambda_keep)
   call pbuf_set_field(pbuf,i_mu,mu_keep)
